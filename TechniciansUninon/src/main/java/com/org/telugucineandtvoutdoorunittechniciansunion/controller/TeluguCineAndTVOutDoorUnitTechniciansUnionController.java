...
import com.org.telugucineandtvoutdoorunittechniciansunion.controller.TeluguCineAndTVOutDoorUnitTechniciansUnionController;
...
public class TeluguCineAndTVOutDoorUnitTechniciansUnionController {
  ...
  public String registrationForm(HttpServletRequest request, Map<String, Object> model) {
    ...
    model.put("PAYMENT_CONF_ID", this.utils.convertPaymentConfigDetailsToHTMLSelect(this.miscellaneousService.getPaymentConfigDetialsForSelect("", "REGISTRATION")));
  ...
  public String updateCardBalanceForm(HttpServletRequest request, Map<String, Object> model) {
    try {
      ...
      ApplicationUtilities.debug(this.getClass(),"******************************************* >>");
      ApplicationUtilities.debug(this.getClass(),"******************************************* >>");
      System.out.println(" CAME TO updateCardBalanceForm  >>");
      if (obj != null) {
        ...
        if (topPanelresultCode != null && !"".equalsIgnoreCase(topPanelresultCode) && 
          ...
          MembershipPayments membershipPayments = this.membershipDAO.getMembershipPaymentsDetails(memberId, transId);
          ApplicationUtilities.debug(this.getClass(),"memberId  "+memberId);
          ApplicationUtilities.debug(this.getClass(),"transId  "+transId);
          if (membershipPayments != null) {
            model.put("PAYMENT_CONF_ID", 
                this.utils.convertPaymentConfigDetailsToHTMLSelected(
                  this.miscellaneousService.getPaymentConfigDetialsForSelect((String)objectTopPanel.get("DEPT_ID"), membershipPayments.getCategory()), membershipPayments.getPaymentConfId()));
            PaymentConfigurations paymentConfigurations = this.miscellaneousService.getPaymentConfigurationsDetailsById(membershipPayments.getPaymentConfId());
       
            System.out.println("getCategory  >> "+ membershipPayments.getCategory());
            System.out.println("getPaymentConfId  >> "+ membershipPayments.getPaymentConfId());
            System.out.println("PAYMENT_CONF_ID  >> "+ model.get("PAYMENT_CONF_ID"));
          ...
          else   ApplicationUtilities.debug(this.getClass(),"IN ELSE ");
        } 
      } 
    } catch (Exception e) {
            ...
            model.put("PAYMENT_CONF_ID", 
                this.utils.convertPaymentConfigDetailsToHTMLSelected(
                  this.miscellaneousService.getPaymentConfigDetialsForSelect(
                    (String)objectTopPanel.get("DEPT_ID"), membershipPayments.getCategory()), membershipPayments.getPaymentConfId()));
            PaymentConfigurations paymentConfigurations = this.miscellaneousService.getPaymentConfigurationsDetailsById(membershipPayments.getPaymentConfId());
            model.put("MEMBERSHIP_AMOUNT", Integer.valueOf(paymentConfigurations.getMembershipAmount()));
            model.put("PAID_AMOUNT", Integer.valueOf(membershipPayments.getPaidAmount()));
            model.put("PAID_DATE", (
